...
public class HangingBasicColumnDungature extends Dungature
{
	private int size;
	private BlockState state;
	
	public HangingBasicColumnDungature(DungatureContext context, int minSize, int maxSize, Supplier<BlockState> stateGetter)
	{
	...
	public void placeColumn(TransformedCoord pair, Room room, IWorld world, Random rand)
	{
		...
		IntStream.range(0, this.size).forEach(i -> world.setBlockState(new BlockPos(pair.finalVec.X, room.getCeilingLevel() - i - 1, pair.finalVec.Y), rotatedState, 2));
	}
