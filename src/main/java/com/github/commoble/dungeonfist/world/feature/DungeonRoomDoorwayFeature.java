...
public class DungeonRoomDoorwayFeature extends Feature<NoFeatureConfig>
{
	public DungeonRoomDoorwayFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactoryIn)
	{
			...
			BlockPos pos, NoFeatureConfig config)
	{
		DungeonChunkGenerator.debugBlocks.ifPresent(blocks -> 
		{
			...
			IntStream.range(0,5).forEach(yLayer -> {
				...
				room.DOORWAY_MAP.keySet().stream()
				.filter(rect -> !rect.intersection(chunkRect).isEmpty())
				//.filter(rect -> rand.nextDouble() < 0.05F)
				.forEach(rect -> {
					room.DOORWAY_MAP.get(rect).place(rect, chunkRect, room, world, rand);
				});
