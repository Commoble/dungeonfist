...
public class TagProvider<T> extends KeyTagProvider<T>
{
	...
	public static <T> TagProvider<T> create(GatherDataEvent event, ResourceKey<Registry<T>> registry)
	{
		return new TagProvider<>(event.getGenerator(), registry, event.getLookupProvider(), ModLoadingContext.get().getActiveContainer().getModId());
	}
